...
###Read data from SQL which indicates adults dental treatment rate by Super Output Area.
...
Dental_work <-  sqlQuery(dbconnection,paste("SELECT * FROM genp.Publication.Dental_TreatmentRate_SOA
                                            where FinancialYear = '2021-22';"))


###Merge Super Output Areas geojson and Dental Treatment Rates for adults data together 
...
addPolygons(data = soa_poly_dental,
  ...
  addLegend(pal = pal_dur, values = range, opacity = 1, title = "Adults who received <br> Filling/Extraction/Crown <br> in 2021/22 per 1000 Registered", position="topright") 


#save map as widget
saveWidget(m, file="DentalworkSOA2022Adults.html",title = "Dental Filling/Extraction/Crown Rate on Adults by SOA 2021/22",   selfcontained = TRUE)
